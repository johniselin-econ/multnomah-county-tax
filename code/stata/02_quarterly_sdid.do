...
if ${use_parallel} == 1 {
	...
	** Open postfile for results accumulation (O(1) per spec)
	local pf_path "${results}sdid/quarterly/quarterly_sdid_results.dta"
	if ${resume} == 1 {
		local pf_path "${results}sdid/quarterly/quarterly_sdid_results_new.dta"
	}
	...
	** Resume mode: merge new results into existing file
	if ${resume} == 1 {
		use "${results}sdid/quarterly/quarterly_sdid_results.dta", clear
		append using "${results}sdid/quarterly/quarterly_sdid_results_new.dta"
		save "${results}sdid/quarterly/quarterly_sdid_results.dta", replace
		capture erase "${results}sdid/quarterly/quarterly_sdid_results_new.dta"
	}
...
foreach out of local all_outcomes {
	...
	** Clean up
	capture graph drop speccurve_`out'
