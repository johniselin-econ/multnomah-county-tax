...
Purpose: Perform synthetic difference-in-difference estimation on quarterly
         ...
         Phase 1 — QCEW (2016 Q1 – 2024 Q4): ln_estabs, ln_avg_wage
...
forvalues y = `start_year'/`end_year' {
	forvalues q = 1/4 {
		...
		** Keep needed variables
		keep fips yq year qtrly_estabs avg_wkly_wage
...
drop if missing(qtrly_estabs) | missing(avg_wkly_wage)
drop if qtrly_estabs <= 0 | avg_wkly_wage <= 0

** Create log outcomes
gen ln_estabs   = ln(qtrly_estabs)
gen ln_avg_wage = ln(avg_wkly_wage)
label var ln_estabs   "Log Quarterly Establishments"
label var ln_avg_wage "Log Avg. Weekly Wage"
...
local qcew_outcomes "ln_estabs ln_avg_wage"
...
local lbl_ln_estabs   "Log Establishments"
local lbl_ln_avg_wage "Log Avg. Weekly Wage"
...
foreach samp of varlist sample_all sample_urban95 sample_urban95_covid sample_demog sample_stringency {
	...
	** Loop over exclusion of 2020
	forvalues exl = 1(-1)0 {
		...
		** Table of results (QCEW: 2 outcomes x 2 covariate settings = 4 columns)
...
if `first_qwi' == 1 {
	...
	** Jump to spec curve section (uses results already saved)
	** We still run spec curves for QCEW outcomes only
	local skip_qwi = 1
}
else {
	local skip_qwi = 0
...
** Create log outcomes
gen ln_emp      = ln(emp_all)
gen ln_emp_ba   = ln(emp_ba)
gen ln_earns    = ln(earns_all)
gen ln_earns_ba = ln(earns_ba)

label var ln_emp      "Log Employment (All)"
label var ln_emp_ba   "Log Employment (BA+)"
label var ln_earns    "Log Avg. Monthly Earnings (All)"
label var ln_earns_ba "Log Avg. Monthly Earnings (BA+)"
...
** Define sample 5: COVID stringency k-means match
qui summ percent_urban if yq == tq(2020q1), de
local p90 = r(p90)
gen urban_top10 = percent_urban >= `p90'
...
local qwi_outcomes "ln_emp ln_emp_ba ln_earns ln_earns_ba"
...
** Short labels
local lbl_ln_emp      "Log Employment (All)"
local lbl_ln_emp_ba   "Log Employment (BA+)"
local lbl_ln_earns    "Log Avg. Earnings (All)"
local lbl_ln_earns_ba "Log Avg. Earnings (BA+)"
...
** All 6 outcomes for spec curves
local all_outcomes "ln_estabs ln_avg_wage ln_emp ln_emp_ba ln_earns ln_earns_ba"
local lbl_ln_estabs   "Log Establishments"
local lbl_ln_avg_wage "Log Avg. Weekly Wage"
local lbl_ln_emp      "Log Employment (All)"
local lbl_ln_emp_ba   "Log Employment (BA+)"
