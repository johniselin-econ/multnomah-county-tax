# =============================================================================
# 02_maps.R
# Map generation for Multnomah County Tax analysis
#
# Adapted from: code/R/map_code.R
# Author: John Iselin
#
# Purpose: Produces all maps used in the paper:
#   1. OR+WA overview with counties and cities
#   2. Multnomah region close-up with Metro boundary
#   3. Combined overview with inset close-up (cowplot)
#   4. US county pool map (contiguous US, in/out sample status)
#   5. Migration flow delta maps
#   6. Rate change maps
#   7. Directional flow maps with hatching
#
# Inputs:
#   - data/working/acs_county_sample.xlsx (from Step 2)
#   - data/working/multnomah_flow_comparison_*.csv (from Step 4)
#   - data/working/multnomah_partner_flows_*.csv (from Step 4)
#   - data/mapping/Metro_District_Boundary/Metro_District_Boundary.shp
#   - tigris county/state/place shapefiles (downloaded automatically)
#
# Outputs:
#   - results/maps/map1.png
#   - results/maps/map2.png
#   - results/maps/map_combined.png
#   - results/maps/map_us_pool.png
#   - results/maps/map_*_*_*_us.png (flow delta maps)
#   - results/maps/map_*_*_*_westcoast.png (flow delta maps)
#   - results/maps/map_*_*_*_rate_change_*.png (rate change maps)
#   - results/maps/map_directional_*_*_*.png (directional flow maps)
# =============================================================================
...
# ------------------------------------------------------------
# File paths (derived from globals)
# ------------------------------------------------------------
filepath1        <- file.path(results_maps, "map1.png")
filepath2        <- file.path(results_maps, "map2.png")
filepath_combined <- file.path(results_maps, "map_combined.png")
filepath_us      <- file.path(results_maps, "map_us_pool.png")

metro_path  <- file.path(data_dir, "mapping", "Metro_District_Boundary", "Metro_District_Boundary.shp")
county_path <- file.path(data_working, "acs_county_sample.xlsx")

# ============================================================
# SECTION 1: MAPS 1-3 (OR/WA overview, close-up, combined)
# ============================================================
...
ggsave(filepath1, map1, width = 10, height = 8, dpi = fig_dpi)
...
ggsave(filepath2, map2, width = 10, height = 8, dpi = fig_dpi)
...
ggsave(filepath_combined, map_combined, width = 16, height = 10, dpi = fig_dpi, bg = "white")
message("Saved: ", filepath_combined)

# ============================================================
# SECTION 2: MAP 4 — US county pool
# ============================================================
...
ggsave(filepath_us, map_us_pool, width = 12, height = 7, dpi = fig_dpi, bg = "white")
...
# ============================================================
# SECTION 3: MIGRATION FLOW DELTA MAPS
...
west_coast_counties <- us_counties_flow <- us_counties |>
  mutate(fips = as.numeric(GEOID))

west_coast_counties <- us_counties_flow |>
  filter(STATEFP %in% west_coast_states)

# Oregon + Washington only (drop California)
orwa_states <- c("41", "53")
...
# Oregon + Washington bounding box
...
for (measure in c("n1", "n2", "agi")) {
 ...
 csv_path <- file.path(data_working, paste0("multnomah_flow_comparison_", measure, ".csv"))
 ...
 for (direction in c("out", "in", "net")) {
   ...
   us_filepath <- file.path(results_maps, paste0("map_", measure, "_", direction, "_us.png"))
   ggsave(us_filepath, us_map, width = 14, height = 9, dpi = fig_dpi, bg = "white")
   ...
   wc_filepath <- file.path(results_maps, paste0("map_", measure, "_", direction, "_westcoast.png"))
   ggsave(wc_filepath, wc_map, width = 8, height = 14, dpi = fig_dpi, bg = "white")
   message(paste0("Saved: ", wc_filepath))

   # ---- Oregon + Washington Map ----
   orwa_map <- create_flow_map(
     data = flow_data,
   ...
   # Save OR+WA map to maps folder
   orwa_filepath <- file.path(results_maps, paste0("map_", measure, "_", direction, "_orwa.png"))
   ggsave(orwa_filepath, orwa_map, width = 8, height = 10, dpi = fig_dpi, bg = "white")
...
for (measure in c("n1", "n2", "agi")) {
 ...
 for (direction in c("out", "in", "net")) {
   ...
   us_rate_filepath <- file.path(results_maps, paste0("map_", measure, "_", direction, "_rate_change_us.png"))
   ggsave(us_rate_filepath, us_rate_map, width = 14, height = 9, dpi = fig_dpi, bg = "white")
   ...
   wc_rate_filepath <- file.path(results_maps, paste0("map_", measure, "_", direction, "_rate_change_westcoast.png"))
   ggsave(wc_rate_filepath, wc_rate_map, width = 8, height = 14, dpi = fig_dpi, bg = "white")
   message(paste0("Saved: ", wc_rate_filepath))

   # ---- Oregon + Washington Map ----
   orwa_rate_map <- create_rate_change_map(
     data = flow_data,
     counties_sf = orwa_counties,
     direction = direction,
     measure = measure,
   ...
   # Save OR+WA rate change map
   orwa_rate_filepath <- file.path(results_maps, paste0("map_", measure, "_", direction, "_rate_change_orwa.png"))
   ggsave(orwa_rate_filepath, orwa_rate_map, width = 8, height = 10, dpi = fig_dpi, bg = "white")
   message(paste0("Saved: ", orwa_rate_filepath))
...
# ============================================================
# SECTION 5: DIRECTIONAL FLOW MAPS (Partner-Normalized)
# ============================================================
...
for (measure in c("n1", "n2", "agi")) {
  ...
  csv_path <- file.path(data_working, paste0("multnomah_partner_flows_", measure, ".csv"))

  # Check if file exists
  if (!file.exists(csv_path)) {
    message(paste0("Warning: Partner flow file not found - ", csv_path))
    message("Run 02_flow_analysis.R first to create partner-normalized flow data")
    next
  }
  ...
  for (direction in c("out", "in")) {
    ...
    # Save US map
    us_filepath <- file.path(results_maps,
                              paste0("map_directional_", measure, "_", direction, "_us.png"))
    ggsave(us_filepath, us_dir_map, width = 14, height = 9, dpi = fig_dpi, bg = "white")
    message(paste0("Saved: ", us_filepath))
    ...
    # Save West Coast map
    wc_filepath <- file.path(results_maps,
                              paste0("map_directional_", measure, "_", direction, "_westcoast.png"))
    ggsave(wc_filepath, wc_dir_map, width = 8, height = 14, dpi = fig_dpi, bg = "white")
    message(paste0("Saved: ", wc_filepath))
    ...
    # Save OR+WA map
    orwa_filepath <- file.path(results_maps,
                                paste0("map_directional_", measure, "_", direction, "_orwa.png"))
    ggsave(orwa_filepath, orwa_dir_map, width = 8, height = 10, dpi = fig_dpi, bg = "white")
    message(paste0("Saved: ", orwa_filepath))
