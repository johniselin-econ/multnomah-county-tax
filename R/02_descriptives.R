...
message("  Exporting LaTeX table...")

irs_tex <- irs_rates |>
  select(state_name, county_name, period, n1_out_rate, n1_in_rate) |>
  mutate(n1_out_rate = n1_out_rate * 100,
         n1_in_rate  = n1_in_rate * 100) |>
  pivot_wider(names_from = period,
              values_from = c(n1_out_rate, n1_in_rate)) |>
  mutate(
    change_net = (n1_in_rate_post_21_22 - n1_out_rate_post_21_22) -
                 (n1_in_rate_pre - n1_out_rate_pre),
    order = case_when(
      county_name == "Multnomah County" ~ 1L,
      state_name == "Oregon"     & county_name == "Washington County" ~ 3L,
      state_name == "Oregon"     & county_name == "Clackamas County"  ~ 4L,
      state_name == "Oregon"     & county_name == "Marion County"     ~ 5L,
      state_name == "Oregon"     & county_name == "Yamhill County"    ~ 6L,
      state_name == "Oregon"     & county_name == "Columbia County"   ~ 7L,
      state_name == "Washington" & county_name == "Clark County"      ~ 9L,
      state_name == "Washington" & county_name == "Skamania County"   ~ 10L,
      state_name == "Oregon"     & county_name == "Other"             ~ 11L,
      state_name == "Washington" & county_name == "Other"             ~ 12L
    ),
    label = case_when(
      state_name == "Oregon"     & county_name == "Other" ~ "All other OR counties",
      state_name == "Washington" & county_name == "Other" ~ "All other WA counties",
...
tex_lines <- c(
  ...
  "% Generated by R/02_descriptives.R",
...
for (i in seq_len(nrow(irs_tex))) {
  ...
  vals <- paste(fmt(row$n1_in_rate_pre), fmt(row$n1_in_rate_post_21_22),
                fmt(row$n1_out_rate_pre), fmt(row$n1_out_rate_post_21_22),
                fmt(row$change_net), sep = " & ")

  indent <- if (row$order %in% c(3:7, 9, 10)) "\\quad " else ""
  tex_lines <- c(tex_lines, paste0(indent, row$label, " & ", vals, " \\\\"))
}
...
tex_lines <- c(tex_lines,
  ...
  "\\small",
  paste0("\\item \\textit{Notes:} Rates are domestic migration flows as a ",
         "percentage of the base filing population (non-movers plus all movers) ",
         "for each county-period. Pre-period averages tax years 2018--2019; ",
         "post-period averages 2021--2022. Change in net rate is the shift in ",
         "net in-migration rate (in-rate minus out-rate) between periods, in ",
         "percentage points. Source: IRS Statistics of Income."),
  "\\end{tablenotes}",
  "\\end{threeparttable}",
  "\\end{table}"
)

writeLines(tex_lines, file.path(results_tables, "table2.tex"))
message("  LaTeX table exported to: ", file.path(results_tables, "table2.tex"))
