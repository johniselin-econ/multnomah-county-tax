# =============================================================================
# 02_sdid_analysis_state.R
# Synthetic Difference-in-Differences estimation — INTERSTATE (IRS5) ONLY
#
# Converted from: code/02_sdid_analysis_state.do
# Author: John Iselin
#
# Purpose: Perform synthetic difference-in-difference estimation for
#          interstate (IRS type 5) migration only. This is a faster subset
#          of 02_sdid_analysis.R that skips domestic IRS and ACS specifications.
#
# Outputs:
#   - sdid_weights_state.rds/xlsx: Synthetic control weights for each spec
#   - sdid_results_state.rds/xlsx: Treatment effects, SEs, p-values for each spec
#   - fig_speccurve_*_irs5.pdf/jpg: Specification curve plots (interstate only)
...
message("=== 02_sdid_analysis_state.R: Starting SDID analysis (IRS5 / Interstate ONLY) ===")
...
## migration variables for type 5 (interstate) plus out types 1 & 2 (denominators)
irs_keep_pattern <- "^(year|fips|state_|county_|.*_out_1$|.*_out_2$|.*_net_5$|.*_in_5$|.*_out_5$)"
irs_data <- irs_data |>
  select(matches(irs_keep_pattern))

## Merge with ACS data (needed for irs_sample_2 definition)
...
## Drop ACS data columns (not needed for IRS5 analysis, only merge indicator)
acs1_cols_to_drop <- names(df)[str_detect(names(df), "^(persons|households|dollars)_") |
                                 str_detect(names(df), "\\.acs1$")]
df <- df |> select(-any_of(acs1_cols_to_drop))

## Merge with ACS college data (needed for irs_sample_2 balanced panel check)
acs2_data <- load_data(file.path(data_working, "acs_county_gross_college"))
df <- df |>
  left_join(
    acs2_data |> select(year, fips),
    by = c("year", "fips")
  ) |>
  mutate(merge_acs_2 = if_else(
    fips %in% unique(acs2_data$fips), 3L, 1L
  ))
...
## =============================================================================
## Define outcome variables (IRS5 / Interstate ONLY)
## =============================================================================
message("  Computing interstate migration rate outcome variables...")
...
## Save prepared panel data
save_data(df, file.path(data_working, "sdid_analysis_data_state"))

# =============================================================================
# SDID ESTIMATION (IRS5 ONLY)
# =============================================================================

message("STEP 2: Running SDID estimation (IRS5 / Interstate only)...")

## Create output directories
dir.create(results_sdid, recursive = TRUE, showWarnings = FALSE)
out_txt_labels <- c("irs5_full_16_22", "irs5_389_16_22")
...
## Define the data/outcome type mapping — IRS5 ONLY
data_specs <- list(
  list(data_var = "irs_sample_1", out_types = c("irs5"),
       covariates = c("population", "per_capita_income")),
  list(data_var = "irs_sample_2", out_types = c("irs5"),
...
make_sdid_table <- function(results_list, data_type, save_path) {
  ...
  ## IRS format (always, since we only run IRS5)
  ...
  tryCatch({
    ...
    ## Header
    header_groups <- c("Returns", "Exemptions", "AGI")
...
## ============================================================================
## MAIN ESTIMATION LOOP (IRS5 ONLY)
## ============================================================================
...
for (i in seq_along(spec_list)) {
  ...
  if (!is.null(out$event)) {
    ...
    y_label <- switch(spec$migr,
      "net" = "Net interstate migration",
      "in"  = "Interstate in-migration",
      "out" = "Interstate out-migration"
    )
...
save_data(all_results, file.path(results_sdid, "sdid_results_state"))

## Export to Excel
tryCatch({
  write.xlsx(all_results, file.path(results_sdid, "sdid_results_state.xlsx"))
  message("  Results saved: sdid_results_state.xlsx")
}, error = function(e) {
...
save_data(all_weights, file.path(results_sdid, "sdid_weights_state"))

## Export weights to Excel
tryCatch({
  write.xlsx(all_weights, file.path(results_sdid, "sdid_weights_state.xlsx"))
  message("  Weights saved: sdid_weights_state.xlsx")
}, error = function(e) {
  warning("Failed to write Excel: ", conditionMessage(e))
})

# =============================================================================
# SPECIFICATION CURVE ANALYSIS (IRS5 ONLY)
# =============================================================================

message("STEP 4: Creating specification curve plots (IRS5 only)...")

if (nrow(all_results) == 0) {
  message("  No successful SDID results — skipping spec curves.")
} else {
...
spec_results <- spec_results |>
  mutate(
    ...
    data_type = case_when(
      str_detect(sample_data, "irs5_389") ~ "IRS (389, Interstate)",
...
## (No preferred specs defined by default for IRS5; modify as needed)
## ============================================================================
...
## ============================================================================
## CREATE SPECIFICATION CURVE PLOTS (IRS5 ONLY)
## ============================================================================
...
for (otype in c("n1", "n2", "agi")) {
  for (migr in c("net", "in", "out")) {
    ...
    p_coef <- ggplot(plot_data, aes(x = spec_rank)) +
      ...
      labs(
        title = paste0(otype_label, ": ", migr_label, " (Interstate)"),
    ...
    ## Lower panel: Specification indicator dots (IRS5 only)
    ...
    ggsave(file.path(results_sdid,
                     paste0("fig_speccurve_", otype, "_", migr, "_irs5.pdf")),
           p_combined, width = 10, height = 8)
    ggsave(file.path(results_sdid,
                     paste0("fig_speccurve_", otype, "_", migr, "_irs5.jpg")),
           p_combined, width = 10, height = 8, dpi = fig_dpi)

    message("    Spec curve plot: ", otype, " / ", migr,
            " (irs5) (", n_specs, " specifications)")
...
message("")
message("==============================================")
message("SDID ANALYSIS (INTERSTATE / IRS5 ONLY) COMPLETE")
message("==============================================")
message("Results saved to:")
message("  - ", file.path(results_sdid, "sdid_results_state.rds"))
message("  - ", file.path(results_sdid, "sdid_weights_state.rds"))
message("  - ", file.path(results_sdid, "irs5_*/tab_sdid_*.tex (tables)"))
message("  - ", file.path(results_sdid, "fig_speccurve_*_irs5.pdf"))
message("==============================================")
